...
\node[draw=red!70, line width=1.2pt, rounded corners=6pt, inner sep=5pt, fit=(Ha) (Ht)] (Hgroup) {};

%% ── ФИЗИКА ───────────────────────────────────────────────
\node[ndphy, below=0.5cm of EMB] (PHY)
{\begin{tabular}{c}Физ. модель\\$F=k\sum n_i^2$\\$\tau=B[n_i^2]^\top$\end{tabular}};
\node[ndphy, below=0.87cm of SSM, xshift=1.0cm] (LOSS)
{\begin{tabular}{c}$\mathcal{L}_{DF}$\end{tabular}};
%% ── СТРЕЛКИ: ИНФЕРЕНС ────────────────────────────────────
...
\draw[redarr] (Hgroup.west) -- (LOSS.east);
\draw[redarrdash] (LOSS.north) -- ++(0,0.4) -| (SSM.south)
node[near start, right, xshift=-165pt, yshift=0pt, font=\footnotesize\bfseries, color=red!70]
